...
from screens.game_over_screen import GameOverScreen # ייבוא מסך הסיום

class BaseTarget:
    """מטרת דמה שמאפשרת לקליעים לזהות את הבסיס כ'יחידה' ולהוריד לו חיים בצורה תקינה"""

    def __init__(self, rect, team, screen):
        self.rect = rect
        self.team = team
        self.screen = screen
        self.is_flying = False  # כדי שקליעים יוכלו לפגוע
        self.unit_type = "base"

    @property
    def hp(self):
        # מחזיר את החיים של הבסיס המתאים
        return self.screen.player_base_hp if self.team == "player" else self.screen.enemy_base_hp

    @hp.setter
    def hp(self, value):
        # מעדכן את החיים של הבסיס ישירות במסך המשחק כשהקליע פוגע
        if self.team == "player":
...
# ==========================================
# מודל הבסיס - מכיל רק את מה שמשותף לכל המודים
# ==========================================
class BaseGameScreen(Screen):
    def __init__(self, game_engine):
        ...
        # --- כוח מיוחד (מכת מחץ) ---
        self.special_btn = Button(550, 121, "assets/images/buttons/special.png", 0.67)  # תצטרך להוסיף תמונה כזו לתיקייה
        self.special_cooldown = 45000  # קולדאון של 45 שניות (במילישניות)
        self.last_special_time = -45000  # שם ערך שלילי כדי שיהיה מוכן מיד בתחילת המשחק
    ...
    def handle_events(self, event):
        ...
        # בדיקה אם סאונד אפקטים (SFX) מאופשר בהגדרות
        sfx_enabled = self.game.settings.get("sfx_on", True)

        # לחיצה על התפתחות לשחקן 1
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.player_age_index < 3 and self.player_xp >= self.max_xps[self.player_age_index]:
                if self.evolve_rect.collidepoint(event.pos):
                    ...
                    # תיקון סאונד Evolve
                    if sfx_enabled and self.click_sound: self.click_sound.play()

        current_player_age = self.ages[self.player_age_index]

        # קניית חיילים לשחקן 1
        if self.buy_melee_btn.is_clicked(event) and self.player_gold >= 100:
            self.player_gold -= 100
            # תיקון סאונד קנייה Melee
            if sfx_enabled and self.click_sound: self.click_sound.play()
            self.units.append(
                Unit(self.player_base_rect.x + 350, self.player_base_rect.bottom - 150, "player", 80, (0, 0, 255),
                     "melee", current_player_age))

        if self.buy_ranged_btn.is_clicked(event) and self.player_gold >= 125:
            self.player_gold -= 125
            # תיקון סאונד קנייה Ranged
            if sfx_enabled and self.click_sound: self.click_sound.play()
            self.units.append(
                Unit(self.player_base_rect.x + 350, self.player_base_rect.bottom - 150, "player", 80, (0, 255, 0),
                     "ranged", current_player_age))

        if self.buy_flying_btn.is_clicked(event) and self.player_gold >= 150:
            self.player_gold -= 150
            # תיקון סאונד קנייה Flying
            if sfx_enabled and self.click_sound: self.click_sound.play()
            self.units.append(
                Unit(self.player_base_rect.x + 350, self.player_base_rect.bottom - 150, "player", 80, (0, 255, 255),
    ...
    def trigger_special_power(self, team="player"):
        """מזמן מטח קליעים מהשמיים על כל חיילי היריב שעל המסך"""
    ...
    def update(self,dt):
                ...
                if p_unit.can_attack(e_unit):
                    ...
                    if current_time - p_unit.last_attack_time > p_unit.attack_speed:
                        ...
                        if p_unit.unit_type in ["ranged", "flying"]:
                            self.projectiles.append(
                                Projectile(p_unit.rect.centerx, p_unit.rect.centery, e_unit, p_unit.damage, p_unit.team,
                                           p_unit.unit_type, p_unit.age))  # <-- הוספנו פה את p_unit.age
                        else:
                ...
                if e_unit.can_attack(p_unit):
                    ...
                    if current_time - e_unit.last_attack_time > e_unit.attack_speed:
                        ...
                        if e_unit.unit_type in ["ranged", "flying"]:
                            self.projectiles.append(
                                Projectile(e_unit.rect.centerx, e_unit.rect.centery, p_unit, e_unit.damage, e_unit.team,
                                           e_unit.unit_type, e_unit.age))  # <-- הוספנו פה את e_unit.age
                        else:
        ...
        for unit in self.units:
            ...
            if target_base_rect:
                ...
                if unit.team == "player":
                    reach_rect = pygame.Rect(target_base_rect.x - stop_dist, target_base_rect.y,
                                             target_base_rect.width + stop_dist, target_base_rect.height)
                else:
                    reach_rect = pygame.Rect(target_base_rect.x, target_base_rect.y,
                                             target_base_rect.width + stop_dist, target_base_rect.height)
                ...
                if unit.rect.colliderect(reach_rect):
                    ...
                    if current_time - unit.last_attack_time > unit.attack_speed:
                        ...
                        if unit.unit_type in ["ranged", "flying"]:
                            ...
                            self.projectiles.append(
                                Projectile(unit.rect.centerx, unit.rect.centery, dummy_base, unit.damage,
                                           unit.team, unit.unit_type, unit.age))  # <-- הוספנו פה את unit.age
                        else:
                            if is_enemy_base:
        ...
        for proj in self.projectiles:
            proj.update(dt)  # <--- להוסיף dt כאן (אם יש פונקציית update ב-Projectile)
            if proj.active:
    ...
    def draw(self):
        # 1. ציור הרקע (השכבה הכי תחתונה)
        self.game.screen.blit(self.bg_img, (0, 0))

        # 2. ציור הדרך (מעל הרקע הראשי, מתחת לחיילים ולבסיס)
        if self.road_img:
        ...
        # ציור חיילים וכדורים
        for unit in self.units: unit.draw(self.game.screen)
        for proj in self.projectiles: proj.draw(self.game.screen)
        # ציור UI שחקן 1
        ...
        # --- ציור כפתור ספיישל עם קולדאון ---
        self.special_btn.draw(self.game.screen)
        current_time = pygame.time.get_ticks()
        time_since_special = current_time - self.last_special_time

        if time_since_special < self.special_cooldown:
            # ציור מלבן שחור חצי-שקוף על הכפתור כדי להראות שהוא לא זמין
        ...
        if self.player_age_index < 3:
            ...
            pygame.draw.rect(self.game.screen, (0, 200, 255),
                             (xp_bar_x, xp_bar_y, xp_bar_width * xp_ratio, xp_bar_height))
